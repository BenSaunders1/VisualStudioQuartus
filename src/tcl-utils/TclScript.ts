...
export class TclScript {
  ...
  constructor(fileName: string, directory: vscode.Uri) {
    ...
    vscode.workspace.fs.writeFile(
      vscode.Uri.joinPath(directory, "tcl-scripts", fileName),
      new Uint8Array(0)
    );
  ...
  async execute(): Promise<string> {
    await vscode.workspace.fs.writeFile(
      vscode.Uri.joinPath(this.directory, "tcl-scripts", this.fileName),
      Buffer.from(this.scriptContents.join("\n"))
    );
    ...
    const stdout = exec(
      "cd " +
        ...
        "\\..\\ && " +
        `quartus_sh -t .\\${PATHS.VSQUARTUS_DIR}\\${PATHS.TCL_SCRIPTS_DIR}\\` +
        this.fileName
    ).stdout;

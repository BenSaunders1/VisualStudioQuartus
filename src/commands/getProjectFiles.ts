...
export async function getProjectFiles() {
  ...
  const script = new TclScript(
    "get_project_files.tcl",
    vscode.Uri.joinPath(project.directory, PATHS.VSQUARTUS_DIR)
  );

  script.addCommand("project_open " + project.projectName);
  script.addCommand(
    "set file_list_v [get_all_global_assignments -name VERILOG_FILE]"
  );
  script.addCommand(
    "foreach_in_collection file $file_list_v { puts [lindex $file 2] }"
  );
  script.addCommand(
    "set file_list_vhdl [get_all_global_assignments -name VHDL_FILE]"
  );
  script.addCommand(
    "foreach_in_collection file $file_list_vhdl { puts [lindex $file 2] }"
  );
  ...
  const files = outputFiles
    .split("\n")
    .map((line) => line.trim())
    .filter((line) => !line.startsWith("Info") && !line.startsWith("Error"));
